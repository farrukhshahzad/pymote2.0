...
def expectation_maximization(t, nbclusters=2, nbiter=3, normalize=False,\
    ...
    -> Draw nbclusters sets of (?, ?, P_{#cluster}) at random (Gaussian
    ...
    [M] * P(Cluster=0)^{t+1} = (1/n).?_{obs} P(Cluster=0|obs)
        * ?^{t+1}_0 = ?_{obs} obs.P(Cluster=0|obs) / P_0
        * ?^{t+1}_0 = ?_{obs} P(Cluster=0|obs)(obs-?^{t+1}_0)^2 / P_0
    -> Compute E_t=?_{obs} log(P(obs)^t)
       Repeat Steps 2 and 3 until |E_t - E_{t-1}| < ?
    """
    def pnorm(x, m, s):
        """
    ...
    for iteration in xrange(nbiter):
        ...
        while (abs(log_estimate - old_log_estimate) > epsilon\
            ...
            print "(EM) old and new log estimate: ",\
                    old_log_estimate, log_estimate, abs(log_estimate - old_log_estimate)
            estimation_round += 1
