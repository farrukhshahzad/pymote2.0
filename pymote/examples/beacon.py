...
class Beacon(NodeAlgorithm):
    ...
    def step(self, node):
        ...
        while True:
            ...
            if message and "RECEIVE" in node.status and node.type != 'C':
    ...
    def send_data(self, node, message):
        ...
        #node.status = 'DONE'
        global tries
        for child_node in self.network.nodes()[1:]:
            #child_node.outbox.insert(0, Message(header="DataPacket", source=child_node,
            #                                    data="Data-" + str(child_node.id)))
            #if 'Information' in child_node.memory:
                child_node.send(Message(header="DataPacket", source=child_node,
